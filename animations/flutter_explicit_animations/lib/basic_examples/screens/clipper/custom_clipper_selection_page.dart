...
class CustomClipperSelectionPage extends StatelessWidget {
  static const id = "CustomClipperSelectionPage";
  ...
  const CustomClipperSelectionPage({Key? key}) : super(key: key);
      ...
      child: Column(
        children: <Widget>[
          const Padding(
            padding: EdgeInsets.all(8.0),
            child: Text(
              "Arc draw 0,0 -> bottom left - 60 -> bottom right - 60",
              style: TextStyle(fontWeight: FontWeight.bold),
            ),
          ),
          Padding(
  ...
  Widget _buildCustomArcTypeClockWise() {
    return Card(
      elevation: 8,
      margin: const EdgeInsets.all(16),
      child: Column(
        children: <Widget>[
          ...
          Padding(
            ...
            child: ClipPath(
              child: Image.network(imagePath),
              clipper: CustomTriangleClipper(ClipType.arcTypeClockWise),
            ),
          ),
        ],
      ),
...
class CustomTriangleClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    } else if (clipType == ClipType.arcTypeClockWise) {
      createArc(size, path, true);
    } else if (clipType == ClipType.arcType) {
      createArc(size, path, false);
...
//    path.lineTo(0, size.height);
  ...
  createArc(Size size, Path path, bool clockwise) {
    if (clockwise) {
      path.lineTo(0, size.height);
      ...
      path.arcToPoint(Offset(size.width, size.height - 60),
          radius: const Radius.elliptical(30, 10), clockwise: false);
      path.lineTo(size.width, 0);
    }
  }
