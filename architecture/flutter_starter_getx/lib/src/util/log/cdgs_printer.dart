...
/// └──────────────────────────
/// ```
class CdgsPrinter extends LogPrinter {
  ...
  late final int stackTraceBeginIndex;
  late final int methodCount;
  late final int errorMethodCount;
  late final int lineLength;
  late final bool colors;
  ...
  String? formatStackTrace(
    ...
    for (var line in lines) {
      if (_discardDeviceStacktraceLine(line) ||
          ...
          _discardCdgsPrinterStacktraceLine(line) ||
          line.isEmpty) {
        continue;
      }

      try {
        final stacktraceStr = line.replaceFirst(RegExp(r'#\d+\s+'), '');
        final stacktraceStrSplit = stacktraceStr.split(' ');
        final filename = '${stacktraceStrSplit[1].split('.dart')[0].split(Platform.pathSeparator).last}.dart';
        final lineOfCode = stacktraceStrSplit[1]
            .split('.dart')[1]
            .replaceFirst(':', '')
            .replaceFirst(')', '');

        formatted.add('$emoji $filename : $lineOfCode');
  ...
  bool _discardCdgsPrinterStacktraceLine(String line) {
    var match = _deviceStackTraceRegex.matchAsPrefix(line);
    if (match == null) {
      return false;
    }
    return match.group(1)!.startsWith('CdgsPrinter.log');
  }
  ...
  List<String> _formatAndPrint(
    ...
    for (var line in message.split('\n')) {
      buffer.add(
          color('$stacktrace ${time ?? ''} $emoji$line'));
